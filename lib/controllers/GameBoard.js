...
export class GameBoard {
    ...
    movePiece(originPoint, destinationPoint) {
        ...
        if(check) this.checkEvent.invoke(that, colorThatMightBeInCheck);
        return message;
    }
    ...
    runPlacementCommand(command) {
        ...
        if (pieceAtLocation !== undefined) throw `${pieceAtLocation.type.name} is already on ${location.Point}`
    ...
    checkIfColorIsInCheck(color){
        ...
        if(color === 'White'){
            ...
            let theOneTrueKing = this.bertPieces.filter((b) => b.type === PieceTypes.King)[0];
            isInCheck = this.whitePieces.filter((w) => w.canCapture(theOneTrueKing)).length > 0;
        }
        return isInCheck;
    }
}