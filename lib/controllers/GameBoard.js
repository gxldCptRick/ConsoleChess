...
export class GameBoard {
    ...
    checkThePiecesAgainForCheckMate(theOneTrueKing, theGuardians, theKillers) {
        let isInCheckMate = this.checkAllPositions(theOneTrueKing, theKillers);
        console.log("checkMate", isInCheckMate);
        if (isInCheckMate) {
            isInCheckMate = theGuardians.filter(guard => this.getPossibleMovesFor(guard.getPosition()).filter(gp => this.dangerZone.includes(gp)).length > 0).length === 0;
        }
        console.log("checkMate", isInCheckMate);
        return isInCheckMate;
    }
    ...
    checkAllPositions(theOneTrueKing, theKillers) {
        ...
        console.log(ogMovments);
        let canBeCapturedInAllPoints = false;
        for (let i = 0; i < theKillers.length && !canBeCapturedInAllPoints; i++) {
            const killer = theKillers[i];
            console.log(ogMovments.filter((a) => killer.canCapture(theOneTrueKing, a)));
